...
고객의 연령 분포를 확인한 결과 전체 고객 중 Y&Z세대가 차지하는 비율은 약 35%에 불과하였다. 그럼에도 불구하고 NH투자증권에서는 Y&Z세대에 대한 이해를 바탕으로 그들을 대상으로 하는 특화 서비스를 마련하고자 한다. 왜일까?
...
cus_info_merged %>% 
  ...
  facet_wrap(~act_opn_ym_1st) + geom_hline(yintercept=50) + ylab('비율(%)') +
...
cus_info_merged %>% 
  ...
  facet_wrap(.~year(act_opn_ym_1st)) + ylab('인원(명)') + theme(axis.title.x=element_blank()) +
...
2020년 월별 신규 고객의 연령 분포를 확인해보면 실제로 신규 고객 수가 코스피 지수가 바닥을 찍었던 2020년 3월에 폭증한 이후 다시 제자리를 찾아가고 있음을 확인할 수 있다. 따라서 NH투자증권은 한 순간에 유입된 대량의 고객들의 충성도를 확보하여 그들이 계속해서 NH투자증권의 서비스를 이용하게 만들어야만 한다. 여기서 주목해야할 점은 이러한 고객들 중 50% 이상이 Y&Z세대에 해당한다는 것이다.
...
```{r}
trd_info %>% 
  mutate(orr_y=factor(year(orr_dt))) %>% 
  group_by(orr_y) %>% 
  summarize(n=n()) %>% 
  ggplot(aes(orr_y, n, fill=orr_y)) + geom_bar(stat='identity') + ylab('거래 횟수') +
  theme(axis.title.x=element_blank()) + geom_text(aes(label=n), vjust=-0.2) +
...
trd_info %>% 
  ...
  theme(axis.title.x=element_blank()) + labs(title='연도별 국내외 거래량', fill='거래년도')
```

놀라운 것은 해외 주식 거래량의 증가폭이다. 반 년의 기간 동안 2019년 한 해 거래량의 다섯 배에 달하는 거래량을 기록하였다. 이는 해외 주식 시장에 투자자들의 이목이 집중되고 있음을 보여준다.
...
trd_info %>% 
  ...
  facet_wrap(~orr_y) + theme(axis.title.x=element_blank()) + ylab('금액(원)') +
  geom_text(aes(label=orr_pr_mean), vjust=-0.3) +
  labs(title='연도별 국내외 주식 평균 거래액', fill='국내/해외')
```

물론 여전히 국내 시장에서의 거래가 전체 거래의 98%를 차지하고 있다. 하지만 장기적인 관점에서 보았을 때, 한 주당 평균 가격이 국내 시장의 4~5배에 달하며, 환전 서비스를 통한 추가 수익을 창출할 수 있는 해외 시장은 증권사들에게 충분히 매력적인 시장이라고 볼 수 있다. NH투자증권에서도 이러한 흐름에 맞추어 지난 1년간 투자지원금 제공, 수수료 우대, 환전 우대 등 해외 주식 거래에 대한 이벤트를 지속적으로 진행해온 것이 확인되었다.

따라서 본 팀은 Y&Z세대의 특성을 도출하는 과정에서 해외 주식 거래와 관련된 부분들에 집중하였다.
...
유일하게 세대간 차이를 확인할 수 있었던 변수는 고객 등급이었는데, 세대간의 경제력 차이를 고려하면 상당히 직관적인 결과라고 생각할 수 있다. '등급 없음'과 '해당 사항 없음'의 경우 '화이트' 등급으로 병합 처리하였다.
...
cus_info_merged %>%
  ...
  facet_wrap(~gen_cd) + ylab('비율(%)') + theme(axis.title.x=element_blank()) +
  labs(title='세대별 고객 성비', fill='성별')
```
...
trd_info %>% 
  ...
  ylab('비율(%)') + theme(axis.title.y=element_blank()) + scale_x_reordered() +
  labs(title='세대별 투자 분야 분포', fill='세대')
```
...
우선 고객 데이터를 기준으로 군집 분석을 실시하기 위해서 고객을 가장 잘 구분지을 수 있는 변수를 탐색하였다. 그 결과로 월 평균 거래 횟수, 거래 주기, 총체결가격최댓값 및 중앙값이 선정되었다. 이후 요인 분석을 통해 두 가지 요인으로 묶어 군집분석의 두 축으로 활용하였는데, 첫 번째 요인은 거래빈도를 나타내고, 두 번째 요인은 경제력을 나타낸다.
...
cus_info_merged %>% 
  ...
  mutate(Cluster=factor(km.out$cluster, levels=c(2, 1, 3, 4),
                        labels=c('가난가끔', '가난자주', '부자가끔', '부자자주'))) %>% 
  group_by(Cluster) %>% 
  summarize(n=n()) %>% 
  mutate(freq=round(n/sum(n), 4)*100) %>% 
  ggplot(aes(Cluster, freq, fill=Cluster)) + geom_bar(stat='identity') +
  theme(axis.title.x=element_blank()) + ylab('비율(%)') + labs(title='군집별 비율', fill='군집')
```

Y&Z세대 내 군집별 비율을 확인해본 결과 가난가끔 그룹이 상대적으로 높은 비율을 차지하긴 하였으나, 큰 차이가 있다고 보기는 어려웠다.
...
cus_info_merged %>% 
  ...
  labs(title='군집별 연령 분포', fill='군집')
```

군집별 연령 분포에서는 가난자주 그룹과 가난가끔 그룹에서는 연령대별로 고른 분포를 보인 반면, 부자가끔 그룹과 부자자주 그룹의 경우에는 나이가 많을 수록 높은 비율을 차지하였다. 이는 나이와 경제력이 비례한다는 직관과 일치하였다.
...
앞선 그래프와 비슷한 맥락에서, 군집별 고객 등급 분포 역시 가난자주 그룹과 가난가끔 그룹에서는 상대적으로 낮은 등급의 비율이 높게 나타났지만, 부자가끔 그룹과 부자자주 그룹에서는 상위 등급도 어느 정도 포진해 있는 것을 확인할 수 있었다.
...
군집별로 월 평균 수수료를 비교한 결과 부자자주 그룹이 가장 많은 수수료를, 가난가끔 그룹이 가장 적은 수수료를 부담하고 있음은 직관적인 결과였다. 하지만 부자가끔 그룹에서 상위 등급의 고객이 차지하는 비율이 상대적으로 높았다는 사실을 고려하였을 때, 가난자주 그룹과 비슷한 양의 수수료를 부담하고 있다는 점은 다소 의아하였다. 이러한 점에 대해서는 이후 분석에서 이유를 찾을 수 있었다.
...
cus_info_merged %>% 
  ...
  labs(title='군집별 최초계좌개설년도 분포', fill='2017/2018') + theme(legend.position='None')
```
...
cus_info_merged %>% 
  ...
  theme(axis.title.x=element_blank()) + labs(title='군집별 이탈 고객 비율', fill='군집')
```

앞서 언급하였듯이 증권사의 입장에서는 신규 고객 유치 뿐만 아니라 기존 고객 유지도 중요하다. 이러한 관점에서 기존 고객의 이탈 여부를 확인할 수 있는 변수가 필요하였다. 이때 마지막 거래가 2020년 5월 이전인 고객들 중에서 거래 주기에 비해 거래 휴식 기간이 비정상적으로 길고 기대 거래 횟수보다 실제 거래 횟수가 적은 고객들을 이탈 고객으로 정의하였다.

군집별로 이탈율을 비교하였을 때 부자가끔 그룹의 이탈율이 가장 높았고 가난자주 그룹의 이탈율이 가장 낮았다. 거래 빈도가 낮을 수록 이탈율이 높게 나타나는 경향을 확인할 수 있었다.

## 군집별 생존 분석
```{r}
fit = survfit(Surv(orr_day, run_away_cd)~Cluster, data=cus_info_merged %>% 
  filter(gen_cd=='Y' | gen_cd=='Z') %>% 
  mutate(Cluster=factor(km.out$cluster, levels=c(1, 2, 3, 4),
                        labels=c('가난자주', '가난가끔', '부자가끔', '부자자주')), 
         orr_day = floor(orr_prd/24), run_away_cd=ifelse(run_away_cd=='이탈', 1, 0)))
ggsurvplot(fit, ylim=c(0.89, 1), censor=F, xlab='Days')
```

추가적으로 군별로 이탈 시점을 자세히 분석하기 위해 생존 분석을 실시하였다. 생존 함수로는 Kapler-Meier 추정법을 사용하였다. 부자가끔 그룹에서는 거래를 시작하고 200일 동안의 이탈 속도가 빨랐다. 부자자주 그룹에서는 거래를 시작한지 300일 이후부터 이탈 속도가 빨라졌다. 가난가끔 그룹과 가난자주 그룹에서는 시점에 상관 없이 이탈 속도가 일정하였다.
...
merge(x=trd_kr_tmp %>% 
  ...
  labs(title='군집별 국내 투자 분야 분포', fill='군집')
```
...
merge(x=cus_info_merged %>% 
        filter(gen_cd=='Y' | gen_cd=='Z') %>% 
        mutate(Cluster=factor(km.out$cluster, levels=c(1, 2, 3, 4),
                              labels=c('가난자주', '가난가끔', '부자가끔', '부자자주'))), y=trd_info %>% 
        filter(gen_cd=='Y' | gen_cd=='Z') %>% 
  ...
  ggplot(aes(Cluster, freq, fill=Cluster)) + geom_bar(stat='identity') +
  theme(axis.title.x=element_blank()) + ylab('비율(%)') +
  labs(title='군집별 해외 거래 비율 비교', fill='군집')
```
...
merge(x=trd_oss_tmp %>% 
  ...
  geom_bar(stat='identity') + coord_flip() + facet_wrap(~Cluster, scales='free_y') +
  scale_x_reordered() + xlab('대분류') + ylab('비율(%)') +
  labs(title='군집별 해외 투자 분야 분포', fill='군집')
```
